...
import { Lock, Shield, ArrowRight, Globe, Map, MessageSquare, X } from 'lucide-react';
...
const ManagementLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      ...
      setError('Login failed. Please verify your credentials or register first.');
    }
  };
  ...
  const handleVerifyOtp = (e) => {
    ...
    if (String(otp).trim() === String(generatedOtp).trim()) {
      if (role === 'central_admin') {
        navigate('/management/central-dashboard');
      } else {
        navigate('/management/state-dashboard');
      }
    } else {
      ...
      <div className="flex flex-col items-center justify-center py-6 min-h-[80vh]">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-300 font-medium tracking-widest uppercase text-sm">
            Central & State Operations Portal
          </p>
        </div>
        ...
        <div className="w-full max-w-md bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden relative">
          ...
          <div className="bg-slate-900/50 p-6 border-b border-white/10 text-center">
            <p className="text-slate-200 font-semibold mb-4 text-sm">Select Management Level</p>
          ...
          {step === 1 ? (
            <form onSubmit={handleSubmit} className="p-8 space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-1">Authority ID</label>
                <input 
                  type="text" 
                  value={id}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-1">Passphrase</label>
                <div className="relative">
                  <input 
                    type="password" 
                    value={password}
                    onChange={(e) => setPassword(e.target.value)}
                    className="w-full pl-10 pr-4 py-3 bg-slate-900/50 border border-slate-600 rounded-lg outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-white placeholder-slate-500" 
                    placeholder="••••••••" 
                  />
              ...
              <div className="text-center mt-4">
                ...
                <Link to="/management/register" className="text-blue-400 hover:text-blue-300 font-bold transition">
                  Enroll Authority
                </Link>
              </div>
            </form>
            ...
            <form onSubmit={handleVerifyOtp} className="p-8 space-y-5 text-center">
              <div className="text-blue-400 mb-4 flex justify-center"><Shield size={48} /></div>
              ...
              <div className="flex gap-4">
                ...
                <button type="submit" className="w-2/3 bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:shadow-blue-500/30 transition">Verify OTP</button>
              </div>
            </form>
          )}
          ...
          <div className="bg-black/30 p-4 text-center">
            <p className="text-slate-400 text-xs font-mono">
              National Agricultural Management Command
            </p>
          </div>
        </div>
      </div>
